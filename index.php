<?php
	...
	*/
	...
	if(get_option('_trackablesharebutton_excerpt') == '1') {
		add_filter('the_excerpt', '_trackableshare_process');
	}
	...
	function _trackableshare_activate() {
		...
		//add_option('_trackablesharebutton_bitlylogin', 'mikestowe');
		//add_option('_trackablesharebutton_bitlykey', 'R_70c42a4f796970c4e23b815721b0b4b7');
	...
	function _trackableshare_process($content) {
		...
		$content .= '<div class="trackable_sharing">';
		...
		foreach($buttons as $button) {
			...
			if($urls[$button]['popup'] != 'false') {
				...
				$content .= '<a href="'.$url.'" style="text-decoration: none; white-space: nowrap;" title="'.ucfirst(substr($button,0,-4)).'"';
			}
			...
			if($google == 1 || $urls[$button]['popup'] != 'false') {
				...
				if($google == '1') {
					$content .= 'that=this;_gaq.push([\'_trackEvent\',\'SocialSharing\',\''.ucfirst(substr($button,0,-4)).'\',that.href]); ';
				}
				
				if($urls[$button]['popup'] != 'false') {
					$jsize = explode('x',$urls[$button]['popup']);
					$content .= 'window.open(\''.$url.'\',\'share\',\'menubar=0,resizable=1,width='.$jsize[0].',height='.$jsize[1].'\'); return false;';
				}
			...
			$content .= '>'.($text == '1'?' '.strtolower(substr($button,0,-4)).' &nbsp; ':'').'</a> ';
		}
	...
	function _trackableshare_admin() {
		...
		if(isset($_POST['buttons'])) {
			...
			update_option('_trackablesharebutton_excerpt', $_POST['excerpt']);
		...
		echo '</h3>
		<input type="radio" name="excerpt" value="1" '.(1 == get_option('_trackablesharebutton_excerpt')?'checked':'').' /> Posts, Pages, and Excerpts &nbsp; <input type="radio" name="excerpt" value="0" '.(0 == get_option('_trackablesharebutton_excerpt')?'checked':'').' /> Posts and Pages only</td></tr>';
		echo '<tr><td colspan="2">&nbsp;</td></tr>';
		...
		if($analytics_error) {
			echo '<br /><span style="font-size: 12px; color: #900;">Unable to detect Google Analytics code.  Please make sure latest Analytics code has been installed.</span>';
		}
		echo '</h3>
	...
	// Developed by Michael Stowe, last updated April 13, 2010
